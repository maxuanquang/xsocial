...
func (a *AuthenticateAndPostService) GetUserFollower(ctx context.Context, info *pb_aap.UserInfo) (*pb_aap.UserFollowerInfo, error) {
...
func (a *AuthenticateAndPostService) GetUserPost(ctx context.Context, info *pb_aap.UserInfo) (*pb_aap.UserPostDetailInfo, error) {
	// Check if the user exists
	...
	err := a.db.Raw("select * from post where user_id = ?", info.GetUserId()).Scan(&postModels).Error
	if err != nil {
		return nil, err
	}
	...
	for _, model := range postModels {
		...
		post, err := a.GetPost(ctx, &pb_aap.PostInfo{PostId: postId})
		if err == nil {
